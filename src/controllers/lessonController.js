...
//   ["admin", "super_admin", "master_admin", "principal", "head_teacher"].includes(role);
...
// const isAdminRole = (role) =>
//   ["admin", "super_admin", "master_admin", "principal", "head_teacher"].includes(
//     role
//   );

// const getStudentLessonAccess = async (student) => {
...
//     armId: enrollment.armId?._id || enrollment.armId,
//   };
// };
...
//     if (!academicUnitId || !title || !sessionId || !termId || !classId || !subjectId) {
//       return res.status(400).json({
//         success: false,
//         message:
//           "Academic unit, title, session, term, class and subject are required",
//       });
//     }
...
//     if (req.body.termId) {
//       const term = await Term.findById(req.body.termId).select("name");
//       lesson.termName = term?.name || lesson.termName || "";
//     }
...
// // exports.getStudentLessons = async (req, res) => {
// //   try {
// //     const { academicUnitId, classId, armId, subjectId, termName, week } =
// //       req.query;
...
// exports.getStudentLessons = async (req, res) => {
//   try {
//     const { subjectId, termName, week } = req.query;
...
//       student.academicUnitId?._id || student.academicUnitId;

//     const classId =
//       student.classId?._id ||
//       student.classId ||
//       student.enrollment?.classId?._id ||
//       student.enrollment?.classId;

//     const armId =
//       student.armId?._id ||
//       student.armId ||
//       student.enrollment?.armId?._id ||
//       student.enrollment?.armId;

//     if (!academicUnitId || !classId) {
//       return res.status(400).json({
//         success: false,
//         message: "Your class information is incomplete. Please contact admin.",
//       });
//     }
...
//       .populate("armId", "name")
//       .populate("subjectId", "name")
//       .sort({ subjectId: 1, termName: 1, week: 1, createdAt: -1 });
...
//       return res.status(404).json({
//         success: false,
//         message: "Lesson not found or not available for your class.",
//       });
//     }
...
const buildStudentLessonQuery = async (req, lessonId = null) => {
  const student = req.student;

  if (!student) {
    return {
      errorStatus: 401,
      errorMessage: "Student not authenticated",
    };
  }

  const access = await getStudentLessonAccess(student);

  if (!access?.academicUnitId || !access?.classId) {
    return {
      errorStatus: 400,
      errorMessage: "Your class information is incomplete. Please contact admin.",
    };
  }

  const query = {
    status: "published",
    academicUnitId: access.academicUnitId,
  ...
  if (access.armId) {
    query.$or = [{ armId: access.armId }, { armId: null }];
  } else {
    query.armId = null;
  }
...
exports.getStudentLessons = async (req, res) => {
  try {
    ...
    const accessResult = await buildStudentLessonQuery(req);

    if (accessResult.errorStatus) {
      return res.status(accessResult.errorStatus).json({
        success: false,
        message: accessResult.errorMessage,
      });
    }

    const query = accessResult.query;
...
exports.getStudentLesson = async (req, res) => {
  try {
    const accessResult = await buildStudentLessonQuery(req, req.params.id);
    ...
    const lesson = await Lesson.findOne(accessResult.query)
      .populate("academicUnitId", "name")
      .populate("sessionId", "name")
      .populate("termId", "name")
      .populate("classId", "name")
...
exports.downloadLessonResource = async (req, res) => {
  try {
    ...
    const accessResult = await buildStudentLessonQuery(req, lessonId);
