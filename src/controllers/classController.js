...
exports.createClass = async (req, res) => {
  try {
    const { name, arms, isGraduatingClass, academicUnitId } = req.body;
    ...
    const duplicateFilter = academicUnitId
      ? { name: normalizedName, academicUnitId }
      : { name: normalizedName };

    const exists = await Class.findOne(duplicateFilter);
    ...
    if (Array.isArray(arms) && arms.length > 0) {
      const uniqueArms = [
        ...new Set(
          arms.map((armName) => String(armName).trim()).filter(Boolean)
        ),
      ];

      const armsToCreate = uniqueArms.map((armName) => ({
        name: armName,
        class: newClass._id,
      }));
...
exports.updateClass = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, isGraduatingClass, academicUnitId } = req.body;
  ...
  try {
    ...
    if (exists) {
      return res.status(400).json({
        message: "Arm already exists in this class",
      });
    }
    ...
    console.error("Add arm error:", err);
...
exports.updateArm = async (req, res) => {
  try {
    ...
    const existingArm = await Arm.findById(id);
    ...
    const duplicate = await Arm.findOne({
      _id: { $ne: id },
      class: existingArm.class,
      name: normalizedName,
    });
    ...
    existingArm.name = normalizedName;
    await existingArm.save();

    res.json(existingArm);
  } catch (err) {
...
exports.deleteArm = async (req, res) => {
  try {
    ...
    const arm = await Arm.findByIdAndDelete(id);
